...
public class IceSwordItem extends SwordItem {
	...
	public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
		...
		if (result) {
			...
			for (int i = 0; i < 20; i++) {
				((ServerLevel) target.level()).sendParticles(TFParticleType.SNOW.value(), target.getX(), target.getY() + target.getBbHeight() * 0.5F, target.getZ(), 1, target.getBbWidth() * 0.5, target.getBbHeight() * 0.5, target.getBbWidth() * 0.5, 0);
			}
		}
