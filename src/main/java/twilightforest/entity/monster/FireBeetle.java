...
public class FireBeetle extends Monster implements IBreathAttacker {
	...
	protected SoundEvent getHurtSound(DamageSource source) {
		return TFSounds.FIRE_BEETLE_HURT.value();
	}

	@Override
	protected SoundEvent getDeathSound() {
		return TFSounds.FIRE_BEETLE_DEATH.value();
	}

	@Override
	protected void playStepSound(BlockPos pos, BlockState block) {
		playSound(TFSounds.FIRE_BEETLE_STEP.value(), 0.15F, 1.0F);
	}
	...
	public void aiStep() {
		...
		if (this.isBreathing()) {
			...
			playSound(TFSounds.FIRE_BEETLE_SHOOT.value(), this.getRandom().nextFloat() * 0.5F, this.getRandom().nextFloat() * 0.5F);
