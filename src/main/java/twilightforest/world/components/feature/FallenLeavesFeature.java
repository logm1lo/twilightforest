...
public class FallenLeavesFeature extends Feature<NoneFeatureConfiguration> {
	...
	private final BlockState state = TFBlocks.FALLEN_LEAVES.value().defaultBlockState();
	...
	private boolean canPlace(BlockPos pos, WorldGenLevel level) {
		...
		return !level.getBlockState(pos).is(this.state.getBlock()) && (level.isEmptyBlock(pos) || level.getBlockState(pos).is(TFBlocks.MAYAPPLE.value()) || level.getBlockState(pos).canBeReplaced()) && (state.is(BlockTags.DIRT) || level.getFluidState(pos.below()).is(Fluids.WATER));
	}
}
