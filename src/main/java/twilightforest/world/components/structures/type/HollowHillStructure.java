...
public class HollowHillStructure extends LandmarkStructure implements ConfigurableSpawns {
    ...
    protected StructurePiece getFirstPiece(GenerationContext context, RandomSource random, ChunkPos chunkPos, int x, int y, int z) {
        return switch (this.size) { // TODO Clean up once TFLandmark params are no longer necessary
            case 1 -> new HollowHillComponent(TFStructurePieceTypes.TFHill.value(), 0, this.size, x - 3, y - 2, z - 3, this.spikePlaceConfig);
            case 2 -> new HollowHillComponent(TFStructurePieceTypes.TFHill.value(), 0, this.size, x - 7, y - 5, z - 7, this.spikePlaceConfig);
            default -> new HollowHillComponent(TFStructurePieceTypes.TFHill.value(), 0, this.size, x - 11, y - 5, z - 11, this.spikePlaceConfig);
        };
    }
