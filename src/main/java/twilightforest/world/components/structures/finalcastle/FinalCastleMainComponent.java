...
public class FinalCastleMainComponent extends TFStructureComponentOld {
	...
	public FinalCastleMainComponent(StructurePieceSerializationContext ctx, CompoundTag nbt) {
		super(TFStructurePieceTypes.TFFCMain.value(), nbt);
	}

	public FinalCastleMainComponent(int i, int x, int y, int z) {
		super(TFStructurePieceTypes.TFFCMain.value(), i, x, y, z);
	...
	public void addChildren(StructurePiece parent, StructurePieceAccessor list, RandomSource rand) {
		...
		buildTowerMaze(list, rand, 48, 0, 24, 60, Direction.SOUTH, TFBlocks.YELLOW_CASTLE_RUNE_BRICK.value().defaultBlockState(), dest);
		...
		buildTowerMaze(list, rand, 0, 30, 24, 60, Direction.NORTH, TFBlocks.BLUE_CASTLE_RUNE_BRICK.value().defaultBlockState(), dest);
	...
	private void buildTowerMaze(StructurePieceAccessor list, RandomSource rand, int x, int y, int z, int howFar, Direction direction, BlockState type, BlockPos dest) {
		if (list instanceof StructurePiecesBuilder start) {
			...
			while (!complete && iterations < 15) {
				...
				FinalCastleMazeTower13Component sTower = new FinalCastleMazeTower13Component(TFStructurePieceTypes.TFFCSiTo.value(), rand, 3, tc.getX(), tc.getY(), tc.getZ(), type, direction);
	...
	private boolean isMazeComplete(StructurePieceAccessor list, BlockState type) {
		if (list instanceof StructurePiecesBuilder start) {
			...
			for (StructurePiece structurecomponent : start.pieces) {
				...
				if (type == TFBlocks.YELLOW_CASTLE_RUNE_BRICK.value().defaultBlockState() && structurecomponent instanceof FinalCastleEntranceTowerComponent) {
					return true;
				}
				if (type == TFBlocks.BLUE_CASTLE_RUNE_BRICK.value().defaultBlockState() && structurecomponent instanceof FinalCastleBellTower21Component) {
					return true;
				}
			}
		}
	...
	public void postProcess(WorldGenLevel world, StructureManager manager, ChunkGenerator generator, RandomSource rand, BoundingBox sbb, ChunkPos chunkPosIn, BlockPos blockPos) {
		...
		BlockState fieldBlock = TFBlocks.PINK_FORCE_FIELD.value().defaultBlockState();
		...
		this.generateBox(world, sbb, 17, 1, 12, 19, 4, 12, TFBlocks.PINK_CASTLE_DOOR.value().defaultBlockState(), AIR, false);
		this.generateBox(world, sbb, 17, 1, 24, 19, 4, 24, TFBlocks.PINK_CASTLE_DOOR.value().defaultBlockState(), AIR, false);
		...
		// door, first floor
		this.generateBox(world, sbb, 48, 1, 23, 48, 4, 25, TFBlocks.YELLOW_CASTLE_DOOR.value().defaultBlockState(), AIR, false);

		// door, second floor
		this.generateBox(world, sbb, 0, 31, 23, 0, 34, 25, TFBlocks.BLUE_CASTLE_DOOR.value().defaultBlockState(), AIR, false);
	}
